...
from warmup.constants import BIRTHDATE, CAMEL, DOMAIN, PORT, SERVER, USERNAME, WRONG_USERNAME_ERROR


def brute_force_email() -> str | None:
    suffixes = list(itertools.chain("".join(suffix) for i in range(1, 3) for suffix in itertools.product(string.ascii_lowercase, repeat=i)))
    ...
    for attempt, suffix in enumerate(suffixes, 1):
        ...
        if (stdout := result.stdout) is not None and (stdout := stdout.strip()) and WRONG_USERNAME_ERROR not in stdout:
            print(stdout)
            return email
